...
- `station.engine.ts`: **capability motoru** — her istasyon `capabilities` JSON'una göre davranır: `qr_generate` (Product ID + QR + etiket/yazdırma), `trolley_read` (Araba Okuma: önceden onaylı AKTİF arabaya ürün işler; trigger'sızsa `nextFreeSlot` otomatik, trigger'lıysa ürün AKTİF olur), `plc_acquire` (PLC Data: ürün AKTİF olur; veri trigger ile — aşağıya bak), `ok_nok` (NOK→`rejected`+alarm), `batch_assign`, `wait_control` (giriş/çıkış + `waitHours`; erken çıkış alarm+reddet), `route_validate` (rota dışı → 409 `ROUTE_VIOLATION`). **Task management:** zorunlu görevler (plc 'done' kaydı/OK-NOK/batch/bekleme) tamamlanmadan `current_step_index` ilerlemez; rota sonunda `completed`. `capturePlcData(stationId)` export — trigger'da data tag'leri AKTİF ürüne yazar + slot atar + ilerletir + aktif ürünü temizler.
- `plc-data-watcher.ts`: **olay-bazlı PLC Data izleyici** — `startPlcDataWatcher()` modül register'da başlar; `reloadPlcDataWatches()` istasyon create/update'de tetiklenir. `workerManager.onData` canlı akışında her plc_acquire istasyonunun `triggerTagId`'sini izler; **false→true kenarında** `capturePlcData` çağırır. Config: `dataTagIds[]` (ürüne yazılacak tag'ler), `triggerTagId` (trigger biti), `slotTagId` (opsiyonel slot/pozisyon).
