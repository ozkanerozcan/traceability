...
- `trace.service.ts`: CRUD + `generateProductId()` (`SH-YYYYMMDD-NNNN`) + slot/record/batch/alarm. `parseCapabilities`/`parseConfig` (JSON kolonlar). **Aktif çalışma bağlamı (bellek-içi):** `getStationContext/setActiveTrolley/setActiveProduct/clearActiveProduct` — istasyonun onaylı arabası + AKTİF ürünü. `getTrolley`/`nextFreeSlot`.
- `station.engine.ts`: **capability motoru** — her istasyon `capabilities` JSON'una göre davranır: `qr_generate` (Product ID + QR + etiket/yazdırma), `trolley_read` (Araba Okuma: önceden onaylı AKTİF arabaya ürün işler; trigger'sızsa `nextFreeSlot` otomatik, trigger'lıysa ürün AKTİF olur), `plc_acquire` (PLC Read — aşağıya bak), `ok_nok` (NOK→`rejected`+alarm), `batch_assign`, `wait_control` (giriş/çıkış + `waitHours`; erken çıkış alarm+reddet), `route_validate` (rota dışı → 409 `ROUTE_VIOLATION`). **Task management:** zorunlu görevler (plc 'done' kaydı/OK-NOK/batch/bekleme) tamamlanmadan `current_step_index` ilerlemez; rota sonunda `completed`.
- **PLC Read (genel, `plc_acquire`):** `capturePlcData(stationId)` — trigger TRUE kenarında `dataTagIds`'i PLC'den okur, Shell ID(ler)e yazar, ilerletir, ardından trigger'ı **false** çeker (handshake → PLC okuma bitti anlar, yeniden trigger edebilir). **Shell ID kaynağı (`config.shellIdSource`):** `'scan'` (taranan AKTİF ürün — varsayılan), `'plc'` (Shell ID `shellIdTagId`'den okunur), `'trolley'` (onaylı arabadaki ürünler — `trolleyMatchMode:'row'`+`rowTagId`+`rowSize`=4 satır-bazlı veya `'all'` tüm ürünler). Aktif ürün yoksa alarm + yine ack.
- **Araba okuma (`trolley_read`) davranışları:** `POST /stations/:key/trolley` onayında araba AKTİF olur (bellek-içi bağlam). **`clearOnRead` (varsayılan true):** yalnız İLK/yükleme istasyonunda önceki içerik `releaseTrolley` ile OTOMATİK temizlenir; sonraki istasyonlar `false`'a çeker (yüklü araba silinmez). slot_count her zaman kalıcı. Frontend kartında canlı shell slot ızgarası (3 sn poll) + `lastCapture` gösterimi.
- `plc-data-watcher.ts`: **olay-bazlı PLC Data izleyici** — `startPlcDataWatcher()` modül register'da başlar; `reloadPlcDataWatches()` istasyon create/update'de tetiklenir. `workerManager.onData` canlı akışında her plc_acquire istasyonunun `triggerTagId`'sini (subscribe — anında algılama) izler; **false→true kenarında** `capturePlcData` çağırır.
- `trace.routes.ts`: `/api/trace/*` — stations, routes (+steps), trolleys, products (+records), `POST /scan` (ana işlem noktası), `POST /stations/:key/trolley` (araba onayı), `GET /stations/:key/context`, `GET /qr/:productId`, `GET /qr-history`, batches, alarms (+ack). `StationError` → 400/404/409/502 zarfı.
- **Yetenekler (güncel):** `qr_generate`, `trolley_read`, `batch_assign`, `ok_nok`, `plc_acquire`, `wait_control`, `alarm`, `operator_confirm`, `route_validate`. (Kaldırıldı: `printing` — qr_generate içerir; `trolley_assign` → `trolley_read`.) **Migration 4** mevcut istasyonları dönüştürür.
- **Frontend StationWorkPage:** her yetenek **ayrı kartta**; trolley_read kartı araba onayını localStorage'da sabit tutar (backend context ile senkron), plc_acquire kartı trigger/data tag özeti + "PLC verisi bekleniyor" durumunu gösterir (3sn context poll).
