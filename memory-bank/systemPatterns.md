...
### Traceability (`modules/traceability/`) — v2 (2026-08-06)
- **Config-bazlı dispatch (station.type KALDIRILDI — Migration 12):** `station.type` DB'den, backend'den ve frontend'den tamamen silindi. Hangi handler'ın çalışacağı `StationConfig` tag varlığına göre belirlenir: `!plcId` → QR üretim; `shellIdTagId + slotTagId` → Trolley-Shell Matching; `shellIdTagId` → Funnel Screwing; `trolleyIdTagId + rowTagId` → Filling; `trolleyIdTagId + dataTagIds[]` → Probing; `trolleyIdTagId` yalnız → TrolleyRead. `trace_stations` tablosu: `id, key, name, sort_order, is_active, config, created_at, updated_at`.
- **2-tag PLC sözleşmesi (`StationConfig`):** `plcId, shellIdTagId, trolleyIdTagId, slotTagId, rowTagId, triggerTagId, dataTagIds[], statusTagId (INT: 0=IDLE/1=BUSY/2=PASS/3=ERROR), clearOnRead, labelWidth/Height`. Eski 5-tag (ackTagId/busyTagId/errorCodeTagId/errorMessageTagId) kaldırıldı. **Satır no ≠ slot no** (1 satır = `row_size` slot — genel ayar, varsayılan 4; 1-tabanlı).
...
- `trace.service.ts` (API wrapper — `Station`, `StationConfig`, `Measurement`, `TriggerPayload` tipleri; `StationType` KALDIRILDI). `QrCode.tsx` (SVG path render). `StationsPage`: **kart grid** görünümü (Add/Delete butonları YOK; istasyonlar sabit; ikon+renk `station.key` alt dizisine göre). `StationWorkPage`: **config-bazlı panel** (`isQr=!config.plcId`, `hasTrolley=!!config.trolleyIdTagId`, `hasShell=!!config.shellIdTagId` vb. ile özel panel seçimi). Ortak bileşenler: `TrolleyGrid` (4x5 slot ızgarası), `MeasurementEditor` (ölçüm ekle/düzenle/sil + önerilen alanlar), `QrLabelModal` (mm boyutlu etiket + yazdırma). `trace/styles/trace.css` (slot grid, panel kartları, kart hover efektleri, measurement editör, qr-label).
